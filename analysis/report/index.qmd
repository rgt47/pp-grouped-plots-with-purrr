...
description: "Generating multiple plots from grouped data programmatically is a job for purrr's map2 and pmap functions. This post walks through the approach step by step using Palmer Penguins."
...
Generating multiple plots from grouped data
programmatically is a recurring task, and the Palmer
Penguins dataset is a good place to work it out. The
task seemed
...
![Functional programming and visualization.](media/images/ambiance1.png){.img-fluid height=3.5in}
...
a y-variable, and a grouping variable (sex) for color
...
#|   smooth and points colored by sex.
#| fig-alt: >
...
- `purrr::pmap` generalizes to arbitrary numbers of
...
- `scale_color_manual` requires the correct number of
  color values matching the levels of the grouping
  variable.
...
- The color palette (purple, green, red) is not
  colorblind-friendly. Production code should use a
...
6. Switch to a colorblind-friendly palette such as
